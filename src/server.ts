...
<body>
  ...
  <script>
    ...
    // Progressive quote state, auto-refresh, quote rendering, transactions, and token balance
    // are now in TypeScript modules (loaded via client.js bundle):
    //   src/client/quote-display.ts, src/client/auto-refresh.ts,
    //   src/client/transactions.ts, src/client/balance.ts
    // Remaining inline JS below consists of:
    //   - Functions not yet extracted (addMevRpcToWallet, formatTokenDisplay, etc.)
    //   - Bridge callbacks (window.__cb_*) for modules that need inline JS state
    //   - DOM element references used by remaining inline functions

    // resetProgressiveQuoteState, cancelInProgressFetches, showProgressiveLoadingState
    // are now in src/client/quote-display.ts

    // renderProgressiveQuote, renderProgressiveError, showProgressiveRecommendation,
    // computeClientRecommendation, fetchQuotesParallel
    // are now in src/client/quote-display.ts
    ...
    // getBestQuoteFromState, populateNonActiveField, clearNonActiveField, scheduleAutoQuote
    // are now in TypeScript modules. window.__cb_* callbacks are set up by client.js bundle.

    // Chain Selector Dropdown is now in src/client/chain-selector.ts (via initChainSelector)

    // Modal DOM elements are managed by src/client/modals.ts (via initModals)
    // Refresh indicator, auto-refresh, and pending swap card are now in TypeScript modules
    ...
    // toHexQuantity, isAddressLike, encodeApproveCalldata, updateTransactionActionStates
    // are now in src/client/transactions.ts
    function isAddressLike(address) {
      return /^0x[a-fA-F0-9]{40}$/.test(String(address || '').trim());
    }

    // Balance shims — delegate to module (loaded via client.js bundle)
    function updateFromTokenBalance() { if (typeof window.updateFromTokenBalance === 'function') window.updateFromTokenBalance(); }
    function updateToTokenBalance() { if (typeof window.updateToTokenBalance === 'function') window.updateToTokenBalance(); }

    // findTokenByAddress shim — delegate to module (loaded via client.js bundle)
    ...
    // window.__cb_getProgressiveQuoteState, __cb_executeSwapFromCard, __cb_getPendingSwapCard,
    // __cb_setPendingSwapCard, __cb_updateTransactionActionStates, __cb_updateTokenBalances,
    // __cb_onWalletConnected, __cb_onWalletDisconnected are now set by client.js bundle (main.ts)
    ...
    // isUserRejectedError is used by addMevRpcToWallet below (kept inline)
    function isUserRejectedError(err) {
      if (!err || typeof err !== 'object') return false;
      if (Number(err.code) === 4001) return true;
      if (err.data && typeof err.data === 'object' && Number(err.data.code) === 4001) return true;
      if (err.error && typeof err.error === 'object' && Number(err.error.code) === 4001) return true;
      return false;
    }
    ...
    // Autocomplete, form, result, submit, tab DOM elements, and URL sync shims
    // are now managed by TypeScript modules (loaded via client.js bundle).
    ...
    // Auto-refresh, tab switching, result display management, token balance functions
    // are now in TypeScript modules: src/client/auto-refresh.ts, src/client/quote-display.ts,
    // src/client/balance.ts (loaded via client.js bundle).


    // applyDefaults is now in src/client/url-sync.ts (called from main.ts)
    // Chain change handler, tab switching, renderQuoteActions, renderSecondaryDetails,
    // renderSpandexQuote, renderCurveQuote, formatCurveRoute, formatErrorWithTokenRefs,
    // handleTokenRefClick, showCompareResult, showError, hasQuoteResults,
    // requestAndRenderCompare, fetchQuotesParallel, runAutoRefreshCycle,
    // transaction functions (isRejectionCode, isUserRejectedError, setTxStatus,
    // setTxCardPending, ensureWalletOnChain, waitForTransactionReceipt,
    // executeCardTransaction, onApproveClick, onSwapClick, executeSwapFromCard),
    // result click handler, runCompareAndMaybeStartAutoRefresh, form submit handler,
    // URL restore, tokenlist init, and second chain change handler
    // are now in TypeScript modules (loaded via client.js bundle).
  </script>
  <script type="module">
