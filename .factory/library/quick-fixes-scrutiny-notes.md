...
- Default tokenlist configuration now uses `DEFAULT_TOKENLISTS` (comma-separated) and falls back to `packages/api/static/tokenlist.json` when unset.
- `TOKENLIST_PATH` is deprecated/replaced by `DEFAULT_TOKENLISTS`.
- The default tokenlist is expected in `packages/api/static/tokenlist.json` (committed). `.factory/init.sh` only fetches a fallback list when needed.
