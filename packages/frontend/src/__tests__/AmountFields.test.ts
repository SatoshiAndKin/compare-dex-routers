...
describe('AmountFields', () => {
  ...
  it('toggle button switches from exactIn to targetOut', async () => {
    const { getByRole } = render(AmountFields);

    expect(formStore.mode).toBe('exactIn');

    const toggleBtn = getByRole('button', { name: /switch direction/i });
    await fireEvent.click(toggleBtn);

    expect(formStore.mode).toBe('targetOut');
  });

  it('toggle button switches from targetOut back to exactIn', async () => {
    formStore.mode = 'targetOut';
  ...
  it('shows targetOut note when in targetOut mode', () => {
    formStore.mode = 'targetOut';

    const { getByText } = render(AmountFields);

    expect(getByText(/reverse quote/i)).toBeTruthy();
  });
});
