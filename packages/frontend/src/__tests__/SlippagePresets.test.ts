...
describe("SlippagePresets", () => {
  ...
  it("custom input updates slippage to custom value", async () => {
    ...
    const customInput = getByPlaceholderText("bps");
  ...
  it("rejects values above 5000 bps (50%)", async () => {
    const { getByPlaceholderText } = render(SlippagePresets);

    const customInput = getByPlaceholderText("bps");
    await fireEvent.input(customInput, { target: { value: "6000" } });

    // Should NOT update to 6000
    expect(formStore.slippageBps).toBe(50);
  });

  it("shows confirmation for values above 1000 bps (10%)", async () => {
    const { getByPlaceholderText, getByText } = render(SlippagePresets);

    const customInput = getByPlaceholderText("bps");
    await fireEvent.input(customInput, { target: { value: "1500" } });

    // Should show warning, not apply yet
    expect(formStore.slippageBps).toBe(50);
